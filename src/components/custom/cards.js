...
import { GoldButton, RegularButton } from "./button"
...
const cardStyles = makeStyles(theme => ({
    ...
    expandedCard: {
        ...
        transition: theme.transitions.create(["all"/*"padding", 'width', 'height', 'box-shadow'*/], {
            duration: theme.transitions.duration.complex,
        }),
    },
    expand: {
...
const CustomCard = React.memo((props) => {
    ...
    const { image = {}, expanded = false, index = {}, cardHeader = {}, cardContent = {}, cardActions = {}, cardMedia = {}, children = {}, data = [] } = props
...
export const CardCarousel = React.memo(({ carouselData, title, id }) => {
    ...
    const ratingStars = React.useCallback((rating, index) => {
        return Array.from(Array(rating).keys()).map((i) => (
            <Star key={i + 'rating-star'} />
        ))
    }, [])

    //interaction
    const [expanded, setExpanded] = React.useState(Array.from(Array(carouselData.length).keys()).map((i) => /*i==0 ? true:*/false))
    //set index in array to true and allow it to be toggled, while all other items remain false
    const handleExpandClick = (index) =>
        setExpanded([...Array(carouselData.length).keys()].map((i) => i === index ? !expanded[i] : false))
                ...
                {carouselData.map((data, index) =>
                    <CustomCard
                        ...
                        cardContent={<>
                            ...
                            <Typography variant="body2" align="left" color="textSecondary" component="p" className={classes.cardTypography}>
                                {data.description}
                            </Typography>
                        </>
                        }
                        ...
                        cardActions={
                            ...
                            >
                                <RegularButton onClick={() => handleExpandClick(index)}>Read more</RegularButton>
                                {data.project && (
                                    <GoldButton onClick={() => routeToBlog(data.project || "/")}>View project</GoldButton>
                                )}
                            </CardActions>
                        }
                        cardMedia={
...
export const BlogPostCard = React.memo((props) => {
    const classes = cardStyles()
    const { title = "default", data = {}, size, cardMedia = {}, cardHeader = {}, cardContent = {}, avatarImg = {} } = props;
    ...
    return (
        <Grid container item xs={size} key={data.title + Math.random()}>
            ...
            <Card className={classes.blogCard} {...props}>
                <CardHeader
                    avatar={
                        <Avatar aria-label="recipe" className={classes.avatar} src={avatarImg && avatarImg}>
                            AJ
                        </Avatar>
                    }
                // action={
                ...
                <CardContent className={classes.blogCardContent}>
                    <Grid container>
                        ...
                        {/*blog post image */}
                        <Grid onClick={() => routeToBlog(data.postsUrl)} item xs={altSize} style={{ order: isAlt ? 1 : 0, pointerEvents: 'all', cursor: 'pointer' }}>
                            <TiltPhaseSix
                                // disabled={!expanded[index]}
                                className={classes.perspectiveModifierInner}>
                                {cardMedia && cardMedia}
                ...
                <CardActions className={classes.blogCardActions}>
                    <GoldButton size="large" onClick={() => routeToBlog(data.postsUrl)}>Read more</GoldButton>
                </CardActions>
