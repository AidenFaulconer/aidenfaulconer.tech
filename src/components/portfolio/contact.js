...
export const Contact = ({
  ...
  const [formSubmitted, toggleFormSubmition] = React.useState(false);
  ...
  const HandleBadInput = (name, formData) => {
    ...
    if (typeof document !== "undefined")
      test
        ? document.getElementById(name).classList.remove("incomplete")
        : document.getElementById(name).classList.add("incomplete");
    return test;
  };

  const SubmitForm = () => {
    //sanitize input
    let testInput = Object.keys(formData).every((data, i) =>
      HandleBadInput(data, formData)
    );
    ...
    >
      {formSubmitted && (
        <div className="messenger">
          ...
          <h1>Thank you!</h1>
          <p>
            Looking forward to getting in touch, you will have a response within
            1-5 business days :)
          </p>
        </div>
      )}
      ...
      <div id="contactMessage">
        ...
        <div className="incomplete-message">
          please write a message containing more than 20 characters
        </div>
        <textarea
          onChange={HandleChange}
          type="hidden"
